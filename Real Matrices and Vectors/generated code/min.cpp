...
TempVector ASL::min(Vector v, Vector u)	// overloaded version 1
{
	...
	*p.ui++ = c;
	...
	v.data.v = u.data.v = 0;
	return w;
} // end function min overloaded version 1


// vectors v and u are assumed to be of the same size.
TempVector ASL::min(TempVector v, Vector u)	// overloaded version 2
{
	unsigned int c;
	anticast p;
	...
	u.data.v = 0;
	return v;
} // end function min overloaded version 2


// vectors v and u are assumed to be of the same size.
TempVector ASL::min(Vector u, TempVector v)	// overloaded version 3
{
	unsigned int c;
	anticast p;
	...
	u.data.v = 0;
	return v;
} // end function min overloaded version 3


// vectors v and u are assumed to be of the same size.
TempVector ASL::min(TempVector v, TempVector u)	// overloaded version 4
{
	unsigned int c;
	anticast p,q;
...
TempVector ASL::min(Vector v, double x)	// overloaded version 6
{
	unsigned int c;
	TempVector u;
	anticast p;
...
TempVector ASL::min(double x, TempVector v)	// overloaded version 7
...
TempVector ASL::min(TempVector v, double x)	// overloaded version 8
{	/* we will recycle the memory used by v. */
	unsigned int c;
	anticast p;
...
TempMatrix ASL::min(Matrix A, Matrix B)	// overloaded version 9
{
	...
	*p.ui++ = *A.data.ui++;	// store the # of rows of A in C.
	...
	return C;
} // end function min overloaded version 9


// matrices A and B are assumed to be of the same size.
TempMatrix ASL::min(TempMatrix A, Matrix B)	// overloaded version 10
{	/* we will recycle the memory used by A. */
	...
	return A;
} // end function min overloaded version 10


// matrices A and B are assumed to be of the same size.
TempMatrix ASL::min(Matrix B, TempMatrix A)	// overloaded version 11
{	/* we will recycle the memory used by A. */
	register unsigned int c;
	register anticast p;
	...
	B.data.v = 0;
	return A;
} // end function min overloaded version 11


// matrices A and B are assumed to be of the same size.
TempMatrix ASL::min(TempMatrix A, TempMatrix B)	// overloaded version 12
{	/* we will recycle the memory used by A. */
	register unsigned int c;
...
TempMatrix ASL::min(Matrix A, double x)	// overloaded version 14
{
	register unsigned int c;
	TempMatrix B;
	register anticast p;
...
TempMatrix ASL::min(double x, TempMatrix A)	// overloaded version 15
{	/* we will recycle the memory used by A. */
	register unsigned int c;
	register anticast p;
...
TempMatrix ASL::min(TempMatrix A, double x)	// overloaded version 16
{	/* we will recycle the memory used by A. */
	register unsigned int c;
	register anticast p;
...
double ASL::min(Vector v) // overloaded version 17
{
	...
	return accumulator;
} // end function min overloaded version 17


// doesn't work on vectors with 0 elements.
double ASL::min(TempVector v) // overloaded version 18
{
	register unsigned int c;
	register anticast p;
	register double accumulator;
...
TempVector ASL::min(Matrix A , unsigned int dim) // overloaded version 19
{
	...
	return v;
} // end function min overloaded version 19


// doesn't work on matrices with 0 elements.
TempVector ASL::min(TempMatrix A , unsigned int dim) // overloaded version 20
{
	register unsigned int reduceDimention,c;
	register anticast source,destination;
	register double accumulator;
